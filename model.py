...
class SentimentRecommendationModel:
    ...
    def __init__(self):
        ...
        self.cleaned_data = pickle.load(open(
            SentimentRecommendationModel.ROOT_PATH + SentimentRecommendationModel.DATA_CLEANED, 'rb'))
        self.lemmatizer = WordNetLemmatizer()
        self.stop_words = set(stopwords.words('english'))

    """GETTING TOP 20 RECOMMENDATION FOR THE USER"""

    def getRecommendationByUser(self, user):
        recommedations = []
        return list(self.user_final_rating.loc[user].sort_values(ascending=False)[0:20].index)

    """GETTING FILTERED RECOMMENDATIONS AND FINDING THE TOP 5 PRODUCTS"""

    def sentiment_recommendations_extract(self, user):
        if (user in self.user_final_rating.index):
            ...
            print(f"USER NAME {user} - NOT FOUND!")
            return None

    """CLASSIFYING SENTIMENT AS 1 or 0 (POSITIVE or NEGATIVE) BY USING THE TRAINED MACHINE LEARNING MODEL"""
    ...
    """TEXT PRE-PROCESSING BEFORE CONNECTING TO MACHING LEARNING MODEL"""
    ...
    """TO FIND THE POS TAG AND DERVIE THE LEMMA FORM"""
    ...
    """TO EXCLUDE STOPWORDS FROM TEXT"""
